...
**New here?** Follow **[docs/FIRST-TIME-SETUP.md](docs/FIRST-TIME-SETUP.md)** (clone → CMake/TurboQuant build → optional Python & Web UI).

Secondary: small **GGUF + Qwen 3.5 hybrid** loader/graph patches so certain checkpoints run (see **[docs/FORK-CHANGES.md](docs/FORK-CHANGES.md)**). **`llama-server`** is built from this same repo (not a separate project)—see **“llama-server with this fork”** in that doc for naming and packaging. **Maintainers:** periodic upstream merges and a canonical **`build/`** pattern are described in **[docs/MAINTAINING-FORK.md](docs/MAINTAINING-FORK.md)**. If you keep a **local folder** with `.cmd` + **`models.ini`** + client snippets (separate from this clone), read **[docs/OPERATOR-RUNTIME.md](docs/OPERATOR-RUNTIME.md)** and the **[examples/models-preset/](examples/models-preset/)** template. Lineage: [TheTom/llama-cpp-turboquant](https://github.com/TheTom/llama-cpp-turboquant) scaffolding; CUDA stack from **`feature/turboquant-kv-cache`** on [spiritbuun/llama-cpp-turboquant-cuda](https://github.com/spiritbuun/llama-cpp-turboquant-cuda), merged into **`master` here** with upstream llama.cpp.
